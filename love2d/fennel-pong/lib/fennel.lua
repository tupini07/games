package.preload["fennel.binary"] = package.preload["fennel.binary"] or function(...)
  ...
  local function execute(cmd)
    local _713_ = os.execute(cmd)
    if (_713_ == 0) then
      return true
    elseif (_713_ == true) then
      return true
    else
      return nil
    end
  ...
  local function compile_binary(lua_c_path, executable_name, static_lua, lua_include_dir, native)
    ...
    local _723_
    do
      local _722_ = shellout((cc .. " -dumpmachine"))
      if (nil ~= _722_) then
        _723_ = _722_:match("mingw")
      else
        _723_ = _722_
      end
    end
    ...
    compile_command = {cc, "-Os", lua_c_path, table.concat(native, " "), static_lua, rdynamic, "-lm", _726_, "-o", (executable_name .. bin_extension), "-I", lua_include_dir, os.getenv("CC_OPTS")}
  ...
  local function native_path_3f(path)
    ...
    if (version_extension and not (version_extension == "") and not version_extension:match("%.%d+")) then
      return false
    else
      local _731_ = extension
      if (_731_ == "a") then
        return path
      elseif (_731_ == "o") then
        return path
      elseif (_731_ == "so") then
        return path
      elseif (_731_ == "dylib") then
        return path
      elseif true then
        local _ = _731_
        return false
  ...
  local function compile(filename, executable_name, static_lua, lua_include_dir, options, args)
    local _let_738_ = extract_native_args(args)
    local modules = _let_738_["modules"]
    local libraries = _let_738_["libraries"]
    local rename_modules = _let_738_["rename-modules"]
...
package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _624_(_241)
      ...
      if ((_G.type(_625_) == "table") and ((_625_).what == "Lua") and (nil ~= (_625_).source) and (nil ~= (_625_).short_src) and (nil ~= (_625_).linedefined)) then
...
local function dosafely(f, ...)
  local args = {...}
  local _684_, _685_ = nil, nil
  local function _686_()
    return f(unpack(args))
  end
  _684_, _685_ = xpcall(_686_, fennel.traceback)
  if ((_684_ == true) and (nil ~= _685_)) then
    local val = _685_
    return val
  elseif (true and (nil ~= _685_)) then
    local _ = _684_
    local msg = _685_
...
for i = #arg, 1, -1 do
  local _694_ = arg[i]
  if (_694_ == "--no-searcher") then
    options["no-searcher"] = true
    table.remove(arg, i)
  elseif (_694_ == "--indent") then
    ...
    table.remove(arg, i)
  elseif (_694_ == "--add-package-path") then
    local entry = table.remove(arg, (i + 1))
    package.path = (entry .. ";" .. package.path)
    table.remove(arg, i)
  elseif (_694_ == "--add-fennel-path") then
    local entry = table.remove(arg, (i + 1))
    fennel.path = (entry .. ";" .. fennel.path)
    table.remove(arg, i)
  elseif (_694_ == "--add-macro-path") then
    local entry = table.remove(arg, (i + 1))
    fennel["macro-path"] = (entry .. ";" .. fennel["macro-path"])
    table.remove(arg, i)
  elseif (_694_ == "--load") then
    handle_load(i)
  elseif (_694_ == "-l") then
    handle_load(i)
  elseif (_694_ == "--no-fennelrc") then
    options.fennelrc = false
    table.remove(arg, i)
  elseif (_694_ == "--correlate") then
    options.correlate = true
    table.remove(arg, i)
  elseif (_694_ == "--check-unused-locals") then
    options.checkUnusedLocals = true
    table.remove(arg, i)
  elseif (_694_ == "--globals") then
    allow_globals(table.remove(arg, (i + 1)), _G)
    table.remove(arg, i)
  elseif (_694_ == "--globals-only") then
    allow_globals(table.remove(arg, (i + 1)), {})
    table.remove(arg, i)
  elseif (_694_ == "--require-as-include") then
    options.requireAsInclude = true
    table.remove(arg, i)
  elseif (_694_ == "--skip-include") then
    ...
    table.remove(arg, i)
  elseif (_694_ == "--use-bit-lib") then
    options.useBitLib = true
    table.remove(arg, i)
  elseif (_694_ == "--metadata") then
    options.useMetadata = true
    table.remove(arg, i)
  elseif (_694_ == "--no-metadata") then
    options.useMetadata = false
    table.remove(arg, i)
  elseif (_694_ == "--no-compiler-sandbox") then
    options["compiler-env"] = _G
    table.remove(arg, i)
  elseif (_694_ == "--raw-errors") then
    options.unfriendly = true
    table.remove(arg, i)
  elseif (_694_ == "--plugin") then
...
local function eval(form)
  local _703_
  if (form == "-") then
    _703_ = (io.stdin):read("*a")
  else
    _703_ = form
  end
  return print(dosafely(fennel.eval, _703_, options))
end
local function compile(files)
  for _, filename in ipairs(files) do
    ...
    do
      local _706_, _707_ = nil, nil
      local function _708_()
        return fennel["compile-string"](f:read("*a"), options)
      end
      _706_, _707_ = xpcall(_708_, fennel.traceback)
      if ((_706_ == true) and (nil ~= _707_)) then
        local val = _707_
        print(val)
      elseif (true and (nil ~= _707_)) then
        local _0 = _706_
        local msg = _707_
        do end (io.stderr):write((msg .. "\n"))
        os.exit(1)
...
if ((_G.type(_710_) == "table") and _711_(...)) then
  return repl()
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "--repl")) then
  return repl()
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "--compile")) then
  local files = {select(2, (table.unpack or _G.unpack)(_710_))}
  return compile(files)
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "-c")) then
  local files = {select(2, (table.unpack or _G.unpack)(_710_))}
  return compile(files)
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "--compile-binary") and (nil ~= (_710_)[2]) and (nil ~= (_710_)[3]) and (nil ~= (_710_)[4]) and (nil ~= (_710_)[5])) then
  local filename = (_710_)[2]
  local out = (_710_)[3]
  local static_lua = (_710_)[4]
  local lua_include_dir = (_710_)[5]
  local args = {select(6, (table.unpack or _G.unpack)(_710_))}
  local bin = require("fennel.binary")
  options.filename = filename
  options.requireAsInclude = true
  return bin.compile(filename, out, static_lua, lua_include_dir, options, args)
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "--compile-binary")) then
  return print((require("fennel.binary")).help)
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "--eval") and (nil ~= (_710_)[2])) then
  local form = (_710_)[2]
  return eval(form)
elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "-e") and (nil ~= (_710_)[2])) then
  local form = (_710_)[2]
  return eval(form)
else
  ...
  if (((_G.type(_710_) == "table") and (nil ~= (_710_)[1])) and _739_(...)) then
    local a = (_710_)[1]
    return print(fennel["runtime-version"]())
  elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "--help")) then
    return print(help)
  elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "-h")) then
    return print(help)
  elseif ((_G.type(_710_) == "table") and ((_710_)[1] == "-")) then
    local args = {select(2, (table.unpack or _G.unpack)(_710_))}
    return dosafely(fennel.eval, (io.stdin):read("*a"))
  elseif ((_G.type(_710_) == "table") and (nil ~= (_710_)[1])) then
    local filename = (_710_)[1]
    local args = {select(2, (table.unpack or _G.unpack)(_710_))}
    arg[-2] = arg[-1]
    arg[-1] = arg[0]
