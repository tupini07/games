...
class ConversationManager extends Process {
	...
	function handle_focused() {
		...
		if (this.current_conversation.is_current_message_finished_displaying && Ctrl.isKeyJustPressed(A)) {
	...
	override function draw() {
		...
		Cam.rect(Cam.off_x, Cam.off_y, Constants.SCREEN_WIDTH, current_height, 14);
		Cam.rect(Cam.off_x, Cam.off_y + Constants.SCREEN_HEIGHT - current_height, Constants.SCREEN_WIDTH, current_height, 14);
		...
		if (this.focus_bar_completion >= 100 && this.current_conversation.is_current_message_finished_displaying) {
			...
			Cam.print(txt, Cam.off_x + Constants.SCREEN_WIDTH - Cam.get_len_of_print(txt) - 8, Cam.off_y + Constants.SCREEN_HEIGHT - 16);
		}
	}
}
